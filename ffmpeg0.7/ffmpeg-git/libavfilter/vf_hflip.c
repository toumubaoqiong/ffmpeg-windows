...
static int config_props(AVFilterLink *inlink)
{
    ...
    const AVPixFmtDescriptor *pix_desc = &(av_getav_pix_fmt_descriptors())[inlink->format];

    av_image_fill_max_pixsteps(flip->max_step, NULL, pix_desc);
    flip->hsub = (av_getav_pix_fmt_descriptors())[inlink->format].log2_chroma_w;
    flip->vsub = (av_getav_pix_fmt_descriptors())[inlink->format].log2_chroma_h;
