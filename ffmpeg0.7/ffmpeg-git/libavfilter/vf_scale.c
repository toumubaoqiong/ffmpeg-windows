...
static int config_props(AVFilterLink *outlink)
{
    ...
    var_values[VAR_HSUB]  = 1<<(av_getav_pix_fmt_descriptors())[inlink->format].log2_chroma_w;
    var_values[VAR_VSUB]  = 1<<(av_getav_pix_fmt_descriptors())[inlink->format].log2_chroma_h;
    ...
    av_log(ctx, AV_LOG_INFO, "w:%d h:%d fmt:%s -> w:%d h:%d fmt:%s flags:0x%0x\n",
           inlink ->w, inlink ->h, (av_getav_pix_fmt_descriptors())[ inlink->format].name,
           outlink->w, outlink->h, (av_getav_pix_fmt_descriptors())[outlink->format].name,
           scale->flags);

    scale->input_is_pal = (av_getav_pix_fmt_descriptors())[inlink->format].flags & PIX_FMT_PAL;
...
static void start_frame(AVFilterLink *link, AVFilterBufferRef *picref)
{
    ...
    scale->hsub = (av_getav_pix_fmt_descriptors())[link->format].log2_chroma_w;
    scale->vsub = (av_getav_pix_fmt_descriptors())[link->format].log2_chroma_h;
