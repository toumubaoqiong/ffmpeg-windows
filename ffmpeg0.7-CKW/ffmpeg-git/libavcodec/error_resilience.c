...
void ff_er_frame_end(MpegEncContext *s)
{
    ...
    {
    	TraceOutMsg("!s->error_recognition %d != 0, || s->error_count %d ==0 || s->avctx->lowres %d != 0 || s->avctx->hwaccel %d != 0",
    		!s->error_recognition,
    		s->error_count,
    		s->avctx->lowres,
    		s->avctx->hwaccel);
    	TraceOutMsg("s->avctx->codec->capabilities&CODEC_CAP_HWACCEL_VDPAU %d != 0, || s->picture_structure != PICT_FRAME %d, || s->error_count==3*s->mb_width*(s->avctx->skip_top + s->avctx->skip_bottom) %d != 0",
    		s->avctx->codec->capabilities&CODEC_CAP_HWACCEL_VDPAU,
