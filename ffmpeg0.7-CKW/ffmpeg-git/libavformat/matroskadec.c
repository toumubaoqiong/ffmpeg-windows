                         ...
                         int max_size, uint64_t *number)
{
    ...
    read = 8 - (av_getff_log2_tab())[total];
    if (read > max_size)
    {
    ...
    /* read out length */
    total ^= 1 << (av_getff_log2_tab())[total];
    while (n++ < read)
...
static int matroska_aac_sri(int samplerate)
{
    ...
    for (sri = 0; sri < av_getsize_ff_mpeg4audio_sample_rates(); sri++)
        if ((av_getff_mpeg4audio_sample_rates())[sri] == samplerate)
            break;
    return sri;
}
