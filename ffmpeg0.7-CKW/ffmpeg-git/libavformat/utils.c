    ...
    for(i = 0; i < ic->nb_streams; i++)
    {
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            if(st->codec->codec_id == CODEC_ID_RAWVIDEO && !st->codec->codec_tag && !st->codec->bits_per_coded_sample)
            {
                ...
                if(ff_find_pix_fmt(av_getff_raw_pix_fmt_tags(), tag) == st->codec->pix_fmt)
...
{
    int ret = 0;
    if(s && ap && 500 == ap->channel)
    {
        //»ñÈ¡socket, ±ØÐëap->channel = 500
        if(!strstr(s->filename, ".sdp")
                && !strstr(s->filename, "rtsp:"))
        {
            return 0;
        }
        ap->width = ap->height = 0;
        int socketInt = 0;
        ...
        for (int i = 0; i < rt->nb_rtsp_streams; i++)
        {
            ...
            if((socketInt = ffurl_get_file_handle(rtsp_st->rtp_handle)) < 0)
            {
                continue;
            }
            switch(i)
