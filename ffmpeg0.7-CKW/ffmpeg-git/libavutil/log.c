...
#else
inline void TraceOutMsgByDebugString(char *strFormat, ...)
{
    va_list args;
    char szText[1024] = {0};
    SYSTEMTIME sys = {0};
    ...
    sprintf(szText,
            ...
            sys.wMilliseconds);
    va_start(args, strFormat);
    _vsnprintf(szText + strlen(szText),
               1024 - strlen(szText),
               strFormat,
               args);
    va_end(args);
...
void TraceOutMsgByDebugStringNULL(char *strFormat, ...)
{
    return ;
}
