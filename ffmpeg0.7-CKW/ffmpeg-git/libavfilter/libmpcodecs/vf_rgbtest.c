...
static void put_pixel(uint8_t *buf, int x, int y, int stride, int r, int g, int b, int fmt)
{
    ...
    case IMGFMT_RGB12:
        ...
        break;
    case IMGFMT_BGR15:
        ((uint16_t *)(buf + y * stride))[x] = ((r >> 3) << 10) | ((g >> 3) << 5) | (b >> 3);
        break;
    case IMGFMT_RGB15:
        ((uint16_t *)(buf + y * stride))[x] = ((b >> 3) << 10) | ((g >> 3) << 5) | (r >> 3);
        break;
    case IMGFMT_BGR16:
        ((uint16_t *)(buf + y * stride))[x] = ((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3);
        break;
    case IMGFMT_RGB16:
        ((uint16_t *)(buf + y * stride))[x] = ((b >> 3) << 11) | ((g >> 2) << 5) | (r >> 3);
        break;
    case IMGFMT_RGB24:
                  ...
                  unsigned int flags, unsigned int outfmt)
{
    if (vf->priv->w > 0)
    {
        d_width  = width  = vf->priv->w;
    }
    if (vf->priv->h > 0)
    {
        d_height = height = vf->priv->h;
    }
