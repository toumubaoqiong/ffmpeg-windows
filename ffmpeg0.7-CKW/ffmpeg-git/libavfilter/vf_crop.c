...
static int config_input(AVFilterLink *link)
{
    ...
    CropContext *crop = ctx->priv;
    const AVPixFmtDescriptor *pix_desc = &(av_getav_pix_fmt_descriptors())[link->format];
    int ret;
    const char *expr;
    double res;
    ...
    crop->hsub = (av_getav_pix_fmt_descriptors())[link->format].log2_chroma_w;
    crop->vsub = (av_getav_pix_fmt_descriptors())[link->format].log2_chroma_h;
