...
static inline void snow_interleave_line_header(int * i, int width, DWTELEM * low, DWTELEM * high){
    (*i) = (width) - 2;
...
static inline void snow_interleave_line_footer(int * i, DWTELEM * low, DWTELEM * high){
    for (; (*i)>=0; (*i)-=2){
        low[(*i)+1] = high[(*i)>>1];
        low[*i] = low[(*i)>>1];
    }
...
static inline void snow_horizontal_compose_lift_lead_out(int i, DWTELEM * dst, DWTELEM * src, DWTELEM * ref, int width, int w, int lift_high, int mul, int add, int shift){
    for(; i<w; i++){
...
static inline void snow_horizontal_compose_liftS_lead_out(int i, DWTELEM * dst, DWTELEM * src, DWTELEM * ref, int width, int w){
        for(; i<w; i++){
