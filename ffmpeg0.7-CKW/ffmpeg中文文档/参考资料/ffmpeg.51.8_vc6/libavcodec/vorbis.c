...
#pragma warning (disable:4018 4244 4047)

extern const uint32_t inverse1[256];
...
// If the codebook is used for (inverse) VQ, calculate codevectors.
        ...
        if (codebook_setup->lookup_type==1) {
            ...
            uint_fast16_t *codebook_multiplicands=malloc(codebook_lookup_values*sizeof(uint_fast16_t));
...
static uint_fast8_t vorbis_floor1_decode(vorbis_context *vc, vorbis_floor_data *vfu, float *vec) {
    ...
    uint_fast16_t *floor1_Y=malloc(vf->x_list_dim*sizeof(uint_fast16_t));
    uint_fast16_t *floor1_Y_final=malloc(vf->x_list_dim*sizeof(uint_fast16_t));
    uint_fast8_t *floor1_flag=malloc(vf->x_list_dim*sizeof(uint_fast8_t));
    uint_fast8_t class_;
    uint_fast8_t cdim;
    uint_fast8_t cbits;
    uint_fast8_t csub;
...
static int vorbis_residue_decode(vorbis_context *vc, vorbis_residue *vr, uint_fast8_t ch, uint_fast8_t *do_not_decode, float *vec, uint_fast16_t vlen) {
    ...
    uint_fast8_t *classifs=malloc(ptns_to_read*vc->audio_channels*sizeof(uint_fast8_t));
    uint_fast8_t pass;
                ...
                for(j_times_ptns_to_read=0, j=0;j<ch_used;++j) {
                    if (!do_not_decode[j]) {
                        ...
                        for(i=0;i<c_p_c;++i) {
                            ...
                            temp2=(((uint_fast64_t)temp) * inverse1[vr->classifications])>>32;
...
static int vorbis_parse_audio_packet(vorbis_context *vc) 
{
    ...
    int_fast32_t i,j;
    uint_fast8_t *no_residue=malloc(vc->audio_channels*sizeof(uint_fast8_t));
    uint_fast8_t *do_not_decode=malloc(vc->audio_channels*sizeof(uint_fast8_t));
    vorbis_mapping *mapping;
    float *ch_res_ptr=vc->channel_residues;
    float *ch_floor_ptr=vc->channel_floors;
    uint_fast8_t *res_chan=malloc(vc->audio_channels*sizeof(uint_fast8_t));
