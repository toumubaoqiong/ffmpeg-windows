...
static int inner_get_watermark_picture(ContextInfo *ci, int cleanup);
static int get_watermark_picture(ContextInfo *ci, int cleanup);
...
#else
    for (optid=1;optid<argc-1;optid++) {
        c = argv[optid][1];
#endif
        switch (c) {
            case 'f':
...
#else
                optid++;
                strncpy(ci->filename, argv[optid], 1999);
#endif
                ci->filename[1999] = 0;
                break;
            case 'm':
...
#ifdef WIN32
    ci->pfni = (FILE_NOTIFY_INFORMATION *)ci->fni_buf;
#endif

    //av_register_all();
    get_watermark_picture(ci, 0);
    return 0;
}
 ...
 * 0 = ok, -1 = error
 ****************************************************************************/
int inner_get_watermark_picture(ContextInfo *ci, int cleanup)
{
...
#ifdef WIN32
static void stop_monitor(ContextInfo *ci)
{
    if (ci->overlap.hEvent != NULL) {
        CloseHandle(ci->overlap.hEvent);
        ci->overlap.hEvent = NULL;
    }

    if (ci->hDirectory != NULL) {
        CloseHandle(ci->hDirectory);
        ci->hDirectory = NULL;
    }
}

static int start_monitor(ContextInfo *ci)
{
    char pathname[200];
    char *p;
    DWORD dwBytes;
    ...
    ci->hDirectory = CreateFile(pathname,
            FILE_LIST_DIRECTORY,
            ...
            NULL);
    if (ci->hDirectory == INVALID_HANDLE_VALUE) {
        av_log(NULL, AV_LOG_ERROR, "Create directory handle failed (0x%lx)\n", GetLastError());
        return -1;
    }

    ci->overlap.hEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
    if (ci->overlap.hEvent == NULL) {
        CloseHandle(ci->hDirectory);
        ci->hDirectory = NULL;
        av_log(NULL, AV_LOG_ERROR, "Create event failed (0x%lx)\n", GetLastError());
        return -1;
    }

    memset(ci->pfni->FileName, 0, sizeof(ci->fni_buf) - sizeof(FILE_NOTIFY_INFORMATION));
    ci->pfni->FileNameLength = sizeof(ci->fni_buf) - sizeof(FILE_NOTIFY_INFORMATION);
    if (!ReadDirectoryChangesW(ci->hDirectory,
                ci->pfni, sizeof(ci->fni_buf),
                FALSE,
        ...
        CloseHandle(ci->hDirectory);
        ci->hDirectory = NULL;
        av_log(NULL, AV_LOG_ERROR, "Read directory changes failed (0x%lx)\n", GetLastError());
        return -1;
    }
...
{
#ifndef WIN32
    return inner_get_watermark_picture(ci, cleanup);
#else
    int ret = 0;
    ...
    if (cleanup)
        ...
        if (!ci->is_done)
            start_monitor(ci);
        else if (GetOverlappedResult(ci->hDirectory, &ci->overlap, &dwBytes, FALSE)/* && ci->pfni->Action == FILE_ACTION_MODIFIED*/) {
            /* check if file name matches */
            char szFileName[1000];
            char *pfilename = NULL;
            
            pfilename = strrchr(ci->filename, '\\');
            if (pfilename != NULL)
                pfilename++;
            else
                pfilename = ci->filename;

            memset(szFileName, 0, sizeof(szFileName));
            WideCharToMultiByte(CP_ACP, 0,
                ci->pfni->FileName, ci->pfni->FileNameLength/2,
                szFileName, sizeof(szFileName),
                NULL, NULL);

            if (strcmp(pfilename, szFileName) == 0) {
                av_log(NULL, AV_LOG_ERROR, "\nwatermark image change detected (0x%lx) in (%s)\n", ci->pfni->Action, ci->filename);
                ret = inner_get_watermark_picture(ci, 1);
            }
    ...
    ret = inner_get_watermark_picture(ci, cleanup);
