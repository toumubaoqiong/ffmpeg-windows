...
static int vorbis_parse_setup_hdr_codebooks(vorbis_context *vc) {
	...
	uint_fast16_t *codebook_multiplicands = NULL;;
...
//            uint_fast16_t codebook_multiplicands[codebook_lookup_values];
			...
			codebook_multiplicands = (uint_fast16_t *)malloc(codebook_lookup_values * sizeof(uint_fast16_t));
...
// Weed out unused vlcs and build codevector vector
			...
			free(codebook_multiplicands);codebook_multiplicands = NULL;
        }
...
static uint_fast8_t vorbis_floor1_decode(vorbis_context *vc, vorbis_floor_data *vfu, float *vec) {
    ...
    uint_fast16_t *floor1_Y = (uint_fast16_t *)malloc(vf->x_list_dim * sizeof(uint_fast16_t));//[vf->x_list_dim];
    uint_fast16_t *floor1_Y_final = (uint_fast16_t *)malloc(vf->x_list_dim * sizeof(uint_fast16_t));//[vf->x_list_dim];
    int *floor1_flag = (int *)malloc(vf->x_list_dim * sizeof(int));//[vf->x_list_dim];
    uint_fast8_t class_;
    uint_fast8_t cdim;
    uint_fast8_t cbits;
    uint_fast8_t csub;
...
// Curve synth - connect the calculated dots and convert from dB scale FIXME optimize ?
	...
	free(floor1_Y);free(floor1_Y_final);free(floor1_flag);
	floor1_Y = NULL;floor1_Y_final = NULL;floor1_flag = NULL;
    return 0;
}
...
static av_always_inline int vorbis_residue_decode_internal(vorbis_context *vc, vorbis_residue *vr, uint_fast8_t ch, uint_fast8_t *do_not_decode, float *vec, uint_fast16_t vlen, int vr_type) {
    ...
    uint_fast8_t *classifs = (uint_fast8_t *)malloc(ptns_to_read*vc->audio_channels * sizeof(uint_fast8_t));//[ptns_to_read*vc->audio_channels];
    uint_fast8_t pass;
    ...
    for(pass=0;pass<=vr->maxpass;++pass) { // FIXME OPTIMIZE?
		...
		uint_fast32_t inverse_class;

        voffset=vr->begin;
        for(partition_count=0;partition_count<ptns_to_read;) {  // SPEC        error
            if (!pass) {
                inverse_class = ff_inverse[vr->classifications];
...
static int vorbis_parse_audio_packet(vorbis_context *vc) {
    ...
    int_fast32_t i,j;
    uint_fast8_t no_residue[/*vc->audio_channels*/5];
    uint_fast8_t do_not_decode[/*vc->audio_channels*/5];
    vorbis_mapping *mapping;
    float *ch_res_ptr=vc->channel_residues;
    float *ch_floor_ptr=vc->channel_floors;
    uint_fast8_t res_chan[/*vc->audio_channels*/5];
                        ...
                        const uint8_t *buf, int buf_size)
{
    vorbis_context *vc = avccontext->priv_data ;
    GetBitContext *gb = &(vc->gb);
    const float *channel_ptrs[/*vc->audio_channels*/5];
    int i;
