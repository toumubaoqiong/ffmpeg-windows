...
static void aspect_to_info(MpegEncContext * s, AVRational aspect){
    ...
    if(aspect.num==0) {aspect.num = 1; aspect.den = 1;}//(AVRational){1,1};}
...
void h263_encode_picture_header(MpegEncContext * s, int picture_number)
{
    ...
    if(s->h263_plus){
        for(i=0; i<2; i++){
            int div, error;
            div= (s->avctx->time_base.num*(uint64_t)1800000L + (uint64_t)500L*s->avctx->time_base.den) / (((uint64_t)1000L+i)*s->avctx->time_base.den);
            div= av_clip(div, 1, 127);
            error= FFABS(s->avctx->time_base.num*(uint64_t)1800000L - ((uint64_t)1000L+i)*s->avctx->time_base.den*div);
            if(error < best_error){
                best_error= error;
                best_divisor= div;
                best_clock_code= i;
