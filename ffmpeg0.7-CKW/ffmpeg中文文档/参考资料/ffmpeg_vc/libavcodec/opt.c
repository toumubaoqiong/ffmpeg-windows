...
static int av_set_number2(void *obj, const char *name, double num, int den, int64_t intnum, const AVOption **o_out){
    ...
    case FF_OPT_TYPE_RATIONAL:
        if((int)num == num) {
			temp.num = num*intnum; temp.den = den;
			*(AVRational*)dst = temp;//= (AVRational){num*intnum, den};
		} else {                
...
const char *av_get_string(void *obj, const char *name, const AVOption **o_out, char *buf, int buf_len){
    ...
    case FF_OPT_TYPE_INT64:     snprintf(buf, buf_len, "%d PRId64", *(int64_t*)dst);break;
...
AVRational av_get_q(void *obj, const char *name, const AVOption **o_out){
    ...
    if(num == 1.0 && (int)intnum == intnum) {
        temp.num = intnum; temp.den = den;
		return temp;//(AVRational){intnum, den};
    } else
