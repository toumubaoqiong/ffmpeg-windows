...
static void adaptive_quantization(MpegEncContext *s, double q){
    int i;
    float *cplx_tab = (float *)malloc(s->mb_num * sizeof(float));//[s->mb_num];
    float *bits_tab = (float *)malloc(s->mb_num * sizeof(float));//[s->mb_num];
...
static int init_pass2(MpegEncContext *s)
{
    ...
    int64_t all_available_bits= (uint64_t)(s->bit_rate*(double)rcc->num_entries/fps);
