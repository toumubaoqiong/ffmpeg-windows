...
//    uint16_t cw[size], syms[size];
	...
	uint16_t *cw = (uint16_t *)malloc(size * sizeof(uint16_t));
	uint16_t *syms = (uint16_t *)malloc(size * sizeof(uint16_t));
	uint8_t *bits2 = (uint8_t *)malloc(size * sizeof(uint8_t));//[size];

	if(!cw || !syms || bits2)exit(0);
...
//	free(cw);free(syms); cw = NULL; syms = NULL;free(bits2);bits2 = NULL;
}
