...
static inline void ls_decode_line(JLSState *state, MJpegDecodeContext *s, void *last, void *dst, int last2, int w, int stride, int comp, int bits){
    ...
    while(x < w) {
        ...
        D2 = Rc - Ra;
        /* run mode */
        if((FFABS(D0) <= state->near_jpeg) && (FFABS(D1) <= state->near_jpeg) && (FFABS(D2) <= state->near_jpeg)) {
            int r;
            int RItype;
...
int ff_jpegls_decode_picture(MJpegDecodeContext *s, int near_jpeg, int point_transform, int ilv){
    int i, t = 0;
