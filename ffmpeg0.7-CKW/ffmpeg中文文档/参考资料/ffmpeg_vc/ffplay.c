...
extern AVFormatContext *avformat_opts ;//= NULL;
extern struct SwsContext *sws_opts ;//= NULL;
extern const char **opt_names ;//= NULL;
static int opt_name_count;
extern AVCodecContext *avctx_opts[CODEC_TYPE_NB];// = {NULL};
...
uint32_t ffplay_audio_time(void) {
	if(cur_stream && cur_stream->audio_st && cur_stream->audio_st->codec) {
		return (float)cur_stream->audio_st->duration / 
				((float)cur_stream->audio_st->time_base.num * 
				(float)cur_stream->audio_st->time_base.den); 
	} else {
		return 0;
	}
}
...
static int packet_queue_put(PacketQueue *q, AVPacket *pkt)
{
	...
	memset(pkt1, 0x00, sizeof(AVPacketList));
    pkt1->pkt = *pkt;
    pkt1->next = NULL;
...
//			break;
//		}
        if (pkt1 && q->first_pkt != q->last_pkt) {
...
//		if(count < 300)
//			SaveData(pkt->data, pkt->size, count);
...
static int audio_decode_frame(VideoState *is, double *pts_ptr)
{
    ...
    int n, len1, data_size, ret, i, flag;
    double pts;
	static const uint8_t compare_temp[] = {0xff, 0xfb, 0x90, 0x64, 0xff, 0x80, 0x15, 0x4a, 0x63, 0xd4, 0xeb, 0x4f};
    for(;;) {
        /* NOTE: the audio packet can contain several frames */
        while (is->audio_pkt_size > 0) {
            data_size = sizeof(is->audio_buf1);
            for(i = 0; i < sizeof(compare_temp); ++i) {
				if(is->audio_pkt_data[i] != compare_temp[i])
					break;
			}
			if(i == (sizeof(compare_temp)))
				flag = 1;
...
//        print_error(is->filename, err);
    ...
    for(;;) {
        ...
        if (is->seek_req) {
            ...
            if(stream_index>=0){
                temp.num = 1; temp.den = AV_TIME_BASE;
...
static void do_exit(void)
{
	...
	free(sws_opts); sws_opts = NULL;
	free(avformat_opts); avformat_opts = NULL;
	
	for(i=0; i<CODEC_TYPE_NB; i++){
        free(avctx_opts[i]);//= avcodec_alloc_context2(i);
		avctx_opts[i] = NULL;
    }
...
// 	input_filename = "D:\\ffmpeg\\ffmpeg_vc\\nv12fang.mpg";
	if (!input_filename) {
    ...
    flags = SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER;//SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER;
    if (SDL_Init (flags)) {
    ...
    event_loop();
	free(cur_stream);cur_stream = NULL;
    /* never returns */
