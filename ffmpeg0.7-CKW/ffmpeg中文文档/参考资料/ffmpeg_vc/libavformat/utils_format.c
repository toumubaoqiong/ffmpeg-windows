                                   ...
                                   AVCodecParserContext *pc, AVPacket *pkt)
{
    ...
    *pden = 0;
    switch(st->codec->codec_type) {
    case CODEC_TYPE_VIDEO:
        if(st->time_base.num*(int64_t)1000L > st->time_base.den){
            *pnum = st->time_base.num;
            *pden = st->time_base.den;
        }else if(st->codec->time_base.num*(int64_t)1000L > st->codec->time_base.den){
                               ...
                               AVCodecParserContext *pc, AVPacket *pkt)
{
    ...
    if(pkt->pts != AV_NOPTS_VALUE && pkt->dts != AV_NOPTS_VALUE && pkt->dts > pkt->pts && st->pts_wrap_bits<63
       /*&& pkt->dts-(1LL<<st->pts_wrap_bits) < pkt->pts*/){
        pkt->dts -= (int64_t)1L<<st->pts_wrap_bits;
    }
...
static int get_std_framerate(int i){
    ...
    else return framerate_temp[i - 60 * 12] * 1000 * 12;//(i-60*12)*1000*12;//((const int[]){24,30,60,12,15})[i-60*12]*1000*12;
}
...
AVStream *av_new_stream(AVFormatContext *s, int id)
{
    ...
    st->sample_aspect_ratio.num = 0;//= (AVRational){0,1};
...
AVProgram *av_new_program(AVFormatContext *ac, int id)
{
    ...
    if(!program){
        ...
        ff_dynarray_add((unsigned long **)(&ac->programs), &ac->nb_programs, (unsigned long)(program));
