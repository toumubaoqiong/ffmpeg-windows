...
static const int ach_temp[4] = {1,  0,  2,  4};
...
static int dv_extract_video_info(DVDemuxContext *c, uint8_t* frame)
{
    ...
    if (c->sys) {
        ...
        avctx->bit_rate = av_rescale_q(c->sys->frame_size, temp,
                                       c->sys->time_base);
...
void dv_offset_reset(DVDemuxContext *c, int64_t frame_offset)
{
    ...
    if (c->ach) {
        temp.num = 8; temp.den = c->ast[0]->codec->bit_rate;
                          ...
                          AVFormatParameters *ap)
{
    ...
    s->bit_rate = av_rescale_q(c->dv_demux->sys->frame_size, temp,
