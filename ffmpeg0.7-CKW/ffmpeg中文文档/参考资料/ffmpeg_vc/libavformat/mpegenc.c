        ...
        if (s->is_vcd) {
            ...
            if (stream->align_iframe || s->packet_number == 0){
                ...
                if (stream->bytes_to_iframe == 0 || s->packet_number == 0) {
                    ...
                    scr += s->packet_size*(int64_t)90000L / (s->mux_rate*(int64_t)50L); //FIXME rounding and first few bytes of each packet
                    size = put_pack_header(ctx, buf_ptr, scr);
                    s->last_scr= scr;
                    buf_ptr += size;
                    /* GOP Start */
...
//av_log(ctx, AV_LOG_DEBUG, "dts:%f pts:%f scr:%f stream:%d\n", timestamp_packet->dts/90000.0, timestamp_packet->pts/90000.0, scr/90000.0, best_i);
    ...
    if (s->is_vcd) {
        ...
        while((vcd_pad_bytes = get_vcd_padding_size(ctx,stream->premux_packet->pts) ) >= s->packet_size){ //FIXME pts cannot be correct here
            ...
            s->last_scr += s->packet_size*(int64_t)90000L / (s->mux_rate*(int64_t)50L); //FIXME rounding and first few bytes of each packet
        }
    }
