...
void av_register_rtp_dynamic_payload_handlers(void)
{
    ...
    ff_register_dynamic_payload_handler(&ff_h263p_dynamic_handler);
    ff_register_dynamic_payload_handler(&ff_h263pp_dynamic_handler);
}

static int rtcp_parse_packet(RTPDemuxContext *s, const unsigned char *buf, int len)
{
    /* patch for stupid Potevio shit's messing up with ntp MSW and LSW */
    int64_t rtcp_ntp_time;
    if (buf[1] != 200)
        return -1;
//    s->last_rtcp_ntp_time = AV_RB64(buf + 8);
    rtcp_ntp_time = AV_RB64(buf + 8);
    /* swap MSW and LSW if ntp time earlier than year 2000 */
    if (rtcp_ntp_time < (INT64_C(0xbbf81e00) << 32)) {
        if ((rtcp_ntp_time & 0xffffffff) > 0xbbf81e00) {
            s->last_rtcp_ntp_time = ((rtcp_ntp_time & 0xffffffff) << 32) | ((rtcp_ntp_time >> 32) & 0xffffffff);
            av_log(NULL, AV_LOG_WARNING, "RTCP: bad ntp timestamp in SR (0x%"PRIx64"), trying to fix (0x%"PRIx64")\n", rtcp_ntp_time, s->last_rtcp_ntp_time);
        } else {
            av_log(NULL, AV_LOG_WARNING, "RTCP: bad ntp timestamp in SR (0x%"PRIx64"), packet ignored\n", rtcp_ntp_time);
            return -1;
        }
    } else
        s->last_rtcp_ntp_time = rtcp_ntp_time;
