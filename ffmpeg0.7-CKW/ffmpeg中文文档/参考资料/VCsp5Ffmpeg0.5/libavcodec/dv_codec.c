...
static inline int dv_rl2vlc(int run, int level, int sign, uint32_t* vlc)
{
    int size;
...
static inline int dv_rl2vlc_size(int run, int level)
{
    int size;
...
#else
static inline int dv_rl2vlc(int run, int l, int sign, uint32_t* vlc)
{
    *vlc = dv_vlc_map[run][l].vlc | sign;
    return dv_vlc_map[run][l].size;
}

static inline int dv_rl2vlc_size(int run, int l)
{
    return dv_vlc_map[run][l].size;
}
#endif
...
static inline PutBitContext* dv_encode_ac(EncBlockInfo* bi, PutBitContext* pb_pool,
...
static inline void dv_set_class_number(DCTELEM* blk, EncBlockInfo* bi,
...
static inline int dv_guess_dct_mode(DCTELEM *blk) {
    DCTELEM *s;
    int score88 = 0;
    int score248 = 0;
    int i;
