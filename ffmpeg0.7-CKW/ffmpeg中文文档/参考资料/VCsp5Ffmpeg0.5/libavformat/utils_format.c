...
/*            if(!st->time_base.num)
    ...
    for(;;) {
        ...
        for(i=0;i<ic->nb_streams;i++) {
            ...
            if(   st->codec->time_base.den >= ((int64_t)101)*st->codec->time_base.num
    ...
    for(i=0;i<ic->nb_streams;i++) {
        ...
        if (st->codec->codec_type == CODEC_TYPE_VIDEO) {
            ...
            if(duration_count[i] && st->codec->time_base.num*((int64_t)101) <= st->codec->time_base.den &&
               st->time_base.num*duration_sum[i]/duration_count[i]*((int64_t)101) > st->time_base.den){
            ...
            if ((st->codec->codec_id == CODEC_ID_MPEG1VIDEO ||
                ...
                if (st->codec_info_nb_frames >= 20) {
                    ...
                    if (coded_frame_rate >= 24.97 &&
                        ...
                        st->r_frame_rate.num = 24000; // = (AVRational){24000, 1001};
